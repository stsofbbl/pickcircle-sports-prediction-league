-- Fix the isolated JHBF test event helpers for teams.sqrt_odds and
-- teams.sqrt_odds_capped, which are generated by Postgres from odds.
...
begin
  ...
  insert into public.teams (
    event_id, name, region, seed, start_round, odds,
    metadata, team_id, school_name, prefecture
  ) values
    (
      v_event_id, '聖光学院', '福島', 1, 2, 1,
      jsonb_build_object('source', 'jhbf-test-harness'),
      'jhbf-test-seiko-gakuin', '聖光学院', '福島'
    ),
    (
      v_event_id, '山梨学院', '山梨', 2, 2, 1,
      jsonb_build_object('source', 'jhbf-test-harness'),
      'jhbf-test-yamanashi-gakuin', '山梨学院', '山梨'
    );
...
begin
  ...
  insert into public.teams (
    ...
    (
      p_event_id, '聖光学院', '福島', 1, 2, 1,
      jsonb_build_object('source', 'jhbf-test-harness'),
      'jhbf-test-seiko-gakuin', '聖光学院', '福島'
    ),
    (
      p_event_id, '山梨学院', '山梨', 2, 2, 1,
      jsonb_build_object('source', 'jhbf-test-harness'),
      'jhbf-test-yamanashi-gakuin', '山梨学院', '山梨'
    );
