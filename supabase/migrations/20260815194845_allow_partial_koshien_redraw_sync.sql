-- Allows QF/SF official redraw cards to be registered incrementally as each matchup is confirmed.
-- Existing completed/scheduled-card conflict checks remain in place.
...
begin
  ...
  if jsonb_typeof(coalesce(p_matches, 'null'::jsonb)) <> 'array' then
    raise exception 'official redraw payload must be an array';
  end if;
  v_provided_matches := jsonb_array_length(p_matches);
  if v_provided_matches < 1 or v_provided_matches > v_expected_matches then
  ...
  if exists (
    ...
    where item->>'round_key' is distinct from p_round_key
      ...
      or (item->>'match_no')::integer not between 1 and v_expected_matches
  ...
  if (select count(distinct (item->>'match_no')::integer) from jsonb_array_elements(p_matches) item) <> v_provided_matches then
    raise exception 'official redraw match numbers must be unique';
  end if;
  ...
  if exists (
    with provided as (
      select (item->>'match_no')::integer match_no, (item->>'team1_id')::uuid team_id
      from jsonb_array_elements(p_matches) item
      union all
      select (item->>'match_no')::integer match_no, (item->>'team2_id')::uuid team_id
      from jsonb_array_elements(p_matches) item
    )
    select 1
    from provided p
    join public.matches m
     ...
     and m.match_no <> p.match_no
     and p.team_id in (m.team1_id, m.team2_id)
  ) then
    raise exception 'official redraw team is already assigned to another match';
  end if;
  ...
  for v_item in select value from jsonb_array_elements(p_matches) loop
    ...
    select t.name into v_team1_name
    from public.teams t where t.id = v_team1_id and t.event_id = p_event_id;
    select t.name into v_team2_name
    from public.teams t where t.id = v_team2_id and t.event_id = p_event_id;
