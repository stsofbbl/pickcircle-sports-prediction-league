...
async function loadState(supabase: DbClient, eventId: string): Promise<DbRow> {
...
async function saveState(supabase: DbClient, eventId: string, patch: DbRow): Promise<void> {
...
async function loadMatches(supabase: DbClient, eventId: string): Promise<DbRow[]> {
  const { data, error } = await supabase
    .from("matches")
...
async function syncSchedule(supabase: DbClient, event: DbRow, reason: string) {
...
async function syncDueResults(supabase: DbClient, event: DbRow, matches: DbRow[], now: Date) {
  const eventId = String(event.id || "");
  const year = eventYear(event);
  ...
  const dueIds = new Set<string>(due.map((match: DbRow) => String(match.id || "")));
...
function resultRetryReady(state: DbRow, now: Date): boolean {
...
Deno.serve(async (req: Request) => {
  ...
  for (const event of Array.isArray(events) ? events : []) {
    ...
    let scheduleSummary: DbRow | null = null;
    ...
    try {
      ...
      if (decision.due) {
        ...
        const todayMatches = matches.filter((match: DbRow) => match.starts_at && jstDateKey(match.starts_at) === today);
        const patch: DbRow = {
      ...
      if (pendingDueMatches(matches, now).length && resultRetryReady(state, now)) {
        ...
        const conflicts = Array.isArray(resultSummary?.conflicts) ? resultSummary.conflicts : [];
        const resultPatch: DbRow = {
          last_error: conflicts.length
            ? `official result conflicts: ${conflicts.map((item: DbRow) => item.reason).join(",")}`
            : null,
